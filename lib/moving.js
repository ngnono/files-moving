...
//                    self.status.errorFileCount += 1;
    ...
    _.forEach(source, function (file) {
        //当前循环 size-1
        //self.status.currentFileCount -= 1;
...
//            self.status.successFileCount += 1;
...
//    if (this.status.tranId === source.tranId &&  this.status.currentDataCount === 0 && this.status.currentFileCount === 0) {
...
//    if (this.status.lastId === this.err.info.tranId && this.status.currentDataCount === 0 && this.status.currentFileCount === 0) {
//        this.emit('end', this.status);
//    }
};
